        ...
        if (!Maintenance.isRuning) {
            ...
            try {
                ...
                if (name.length() <= 10 && name.length() >= 5) {
                    ...
                    org.bson.Document doc = collection.find(com.mongodb.client.model.Filters.or(
                            com.mongodb.client.model.Filters.eq("name", name),
                            com.mongodb.client.model.Filters.eq("account_id", session.userId)
                    )).first();
                    ...
                    if (doc != null) {
                        ...
                        if (Util.haveSpecialCharacter(name)) {
                            ...
                            if (isNotIgnoreName) {
                                created = PlayerService.gI().createPlayer(session.userId, name.toLowerCase(), gender, hair);
                            }
                        }
                    }
                } else {
